...
import com.taufiqrahman.reviewratings.utils.ViewUtils;
...
public class RatingReviews extends FrameLayout {
  ...
  private void draw(int dimension, Bar initialBar, final Bar bar, final View view) {
    ...
    view.post(new Runnable() {
      @Override public void run() {
        ...
        if(isRoundCorner){
          view.findViewById(R.id.linear_bar).setBackground(ViewUtils.getRoundedBarDrawable(bgColor,radius));
        }else{
   ...
   */
  @SuppressWarnings("deprecation") private void getDimension(final View view,
    ...
    if (bar != null) {
      if (mLinearParentLayout.getHeight() == 0) {
        getDimension(mLinearParentLayout, new DimensionReceivedCallback() {
          @Override public void onDimensionReceived(int dimension) {
